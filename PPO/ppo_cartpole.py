...
# seed_generator = tf.keras.utils.set_random_seed(1337)
...
ckpt_manager = tf.train.CheckpointManager(ckpt, ckptdir, max_to_keep=5)
...
for epoch in range(epochs):
    ...
    # Write to .json file
    with open("ppo_training_log.json", "w") as f:
        json.dump(metrics_log, f, indent=4)
    
    # Write to tensorboard
    with writer.as_default():
        tf.summary.scalar("Mean Return", mean_return, step=epoch)
        tf.summary.scalar("Mean Episode Length", mean_length, step=epoch)
        writer.flush()
